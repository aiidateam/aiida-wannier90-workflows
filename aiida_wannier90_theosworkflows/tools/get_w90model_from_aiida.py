...
def get_pythtb(wan_calculation, xyz_workaround = False):
    ...
    for filepath in [
        os.path.join(wan_calculation.folder.abspath, "raw_input", "{}.win".format(seedname)),
    ...
    if xyz_workaround:
        parsed = raw_wout_parser(open(os.path.join(out_dir, "{}.wout".format(seedname))).readlines())
        ...
        for wf in parsed['wannier_functions_output']:
            xyz.append("X {} {} {}".format(wf['coordinates'][0], wf['coordinates'][1], wf['coordinates'][2]))
        for site in wan_calculation.inp.structure.sites:
            xyz.append("{} {} {} {}".format(site.kind_name, site.position[0], site.position[1], site.position[2]))
