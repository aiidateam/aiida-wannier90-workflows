...
def test_scdm(
    generate_workchain_wannier90, fixture_localhost, generate_remote_data, generate_bands_data,
    generate_projection_data, generate_calc_job_node
    ...
    remote.store()
    remote.add_incoming(scf_workchain, link_type=LinkType.RETURN, link_label='remote_folder')

    params = orm.Dict(dict={'fermi_energy': 6.0, 'number_of_electrons': 8})
    params.store()
    params.add_incoming(scf_workchain, link_type=LinkType.RETURN, link_label='output_parameters')
    ...
    remote = generate_remote_data(computer=fixture_localhost, remote_path='/path/on/remote')
    remote.store()
    remote.add_incoming(nscf_workchain, link_type=LinkType.RETURN, link_label='remote_folder')
    ...
    pw_input_helper(nscf_workchain.inputs.pw.parameters.get_dict(), nscf_workchain.inputs.pw.structure)
    ...
    bands_data.store()
    bands_data.add_incoming(projwfc_workchain, link_type=LinkType.RETURN, link_label='bands')

    # Set 8 orbitals for workchain.sanity_check()
    projection_data = generate_projection_data(8)
    projection_data.store()
    projection_data.add_incoming(projwfc_workchain, link_type=LinkType.RETURN, link_label='projections')
    ...
    calcjob = generate_calc_job_node(
        entry_point_calc_job, fixture_localhost, inputs={'parameters': orm.Dict()}, store=False
    )
    calcjob.set_process_state(ProcessState.FINISHED)
    calcjob.set_exit_status(0)
    calcjob.add_incoming(workchain.inputs.structure, link_type=LinkType.INPUT_CALC, link_label='structure')
    calcjob.add_incoming(w90pp_workchain, link_type=LinkType.CALL_CALC, link_label='iteration_01')
    calcjob.store()
    ...
    nnkp_file.store()
    nnkp_file.add_incoming(w90pp_workchain, link_type=LinkType.RETURN, link_label='nnkp_file')
    ...
    remote = generate_remote_data(computer=fixture_localhost, remote_path='/path/on/remote')
    remote.store()
    remote.add_incoming(pw2wan_workchain, link_type=LinkType.RETURN, link_label='remote_folder')
    ...
    remote = generate_remote_data(computer=fixture_localhost, remote_path='/path/on/remote')
    remote.store()
    remote.add_incoming(w90_workchain, link_type=LinkType.RETURN, link_label='remote_folder')
