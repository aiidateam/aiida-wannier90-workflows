#!/usr/bin/env python
"""Run a ``Wannier90BandsWorkChain`` for Wannier90 band structure with external projector and spin-orbit coupling.
...
def submit(  # pylint: disable=too-many-positional-arguments
    ...
    """Submit a ``Wannier90BandsWorkChain`` to calculate Wannier bands with spin-orbit coupling."""
...
def cli(
    ...
    """Run a ``Wannier90BandsWorkChain`` with external projectors and spin-orbit coupling.

    FILENAME: a crystal structure file, e.g., ``input_files/GaAs.xsf``.
    PSEUDO_FAMILY: label of pseudo family, e.g., ``PseudoDojo/0.4/PBE/FR/standard/upf``.
    EXTERNAL_PROJECTORS_PATH: the path to the directory on computing node which includes the external projectors.
    e.g., ``input_files/external_projectors_soc/``
    """
