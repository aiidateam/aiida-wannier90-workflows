...
def compute_checkerboard(optimize_workchain: Wannier90OptimizeWorkChain) -> ty.Tuple[np.array, np.array, np.array]:
    ...
    wannier_base = optimize_workchain.get_outgoing(link_label_filter='wannier90').one().node
...
def plot_checkerboard(optimize_workchain: Wannier90OptimizeWorkChain, filename: str = None) -> None:
