...
Wannier90OpengridWorkChain = WorkflowFactory('wannier90_workflows.opengrid')


@pytest.fixture
def get_bands_generator_inputs(fixture_code, generate_structure):
    """Generate a set of default inputs for the ``Wannier90OpengridWorkChain.get_builder_from_protocol()`` method."""
...
def test_scdm(get_bands_generator_inputs, data_regression, serialize_builder, structure):
    ...
    builder = Wannier90OpengridWorkChain.get_builder_from_protocol(**inputs)
...
def test_electronic_type(get_bands_generator_inputs):
    ...
    builder = Wannier90OpengridWorkChain.get_builder_from_protocol(
        **get_bands_generator_inputs(), electronic_type=ElectronicType.INSULATOR, opengrid_only_scf=False
    )
    ...
    builder = Wannier90OpengridWorkChain.get_builder_from_protocol(
        **get_bands_generator_inputs(), electronic_type=ElectronicType.METAL, opengrid_only_scf=False
    )
...
def test_spin_type(get_bands_generator_inputs):
    ...
    builder = Wannier90OpengridWorkChain.get_builder_from_protocol(
        **get_bands_generator_inputs(), spin_type=SpinType.NONE, opengrid_only_scf=False
    )
