...
class Wannier90WorkChain(ProtocolMixin, WorkChain):  # pylint: disable=too-many-public-methods
    ...
    def define(cls, spec):
        ...
        spec.expose_outputs(ProjwfcBaseWorkChain, namespace='projwfc', namespace_options={'required': False})
        ...
        spec.exit_code(420, 'ERROR_SUB_PROCESS_FAILED_SCF', message='the scf PwBaseWorkChain sub process failed')
        spec.exit_code(430, 'ERROR_SUB_PROCESS_FAILED_NSCF', message='the nscf PwBaseWorkChain sub process failed')
        spec.exit_code(440, 'ERROR_SUB_PROCESS_FAILED_PROJWFC', message='the ProjwfcBaseWorkChain sub process failed')
        spec.exit_code(
            450,
    ...
    def get_builder_from_protocol(  # pylint: disable=unused-argument,too-many-statements
        ...
        *,
        ...
        from aiida_wannier90_workflows.utils.workflows.builder import (
            check_codes, guess_wannier_projection_types, recursive_merge_builder
        )
        from aiida_wannier90_workflows.utils.workflows.builder.generator import (get_scf_builder, get_nscf_builder)
        from aiida_wannier90_workflows.utils.pseudo import get_pseudo_orbitals, get_semicore_list
        ...
        projection_type, disentanglement_type, frozen_type = guess_wannier_projection_types(
        ...
        if pseudo_family is None:
            if spin_type == SpinType.SPIN_ORBIT:
                ...
                pseudo_family = Wannier90BaseWorkChain.get_protocol_inputs(protocol=protocol
        ...
        builder['wannier90'] = wannier_builder._inputs(prune=True)  # pylint: disable=protected-access

        kpoints_distance = Wannier90BaseWorkChain.get_protocol_inputs(protocol=protocol, overrides=wannier_overrides
        ...
        builder['scf'] = scf_builder._inputs(prune=True)  # pylint: disable=protected-access
        ...
        if retrieve_hamiltonian:
            builder = recursive_merge_builder(builder, protocol_overrides['retrieve_hamiltonian'])
        if retrieve_matrices:
            builder = recursive_merge_builder(builder, protocol_overrides['retrieve_matrices'])
