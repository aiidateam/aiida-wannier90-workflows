...
class Wannier90WorkChain(
    ...
    def get_builder_from_protocol(  # pylint: disable=unused-argument
        ...
        *,
        ...
        # Skipped entirely when `nscf_parent_folder` reuses an nscf run
        # performed elsewhere; the scf half is skipped on its own when
        # `scf_parent_folder` reuses just the scf run.
        if run_scf or run_nscf:
            ...
            scf_builder, nscf_builder = cls.get_scf_nscf_builders_from_protocol(
                ...
                protocol=protocol,
                overrides={
                    key: inputs[key] for key in ("scf", "nscf") if key in inputs
                },
                electronic_type=electronic_type,
