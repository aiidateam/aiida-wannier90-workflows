...
def get_unique_magmoms(sites: list) -> dict:
    ...
    for site in sites:
        ...
        if any(np.linalg.norm(magmom - m)<_MAGMOM_THRESHOLD for m in unique_magmoms[symbol]):
            continue
        else:
            ...
            if hasmag:
                ...
                if len(unique_magmoms[site.specie.symbol]) == 1:
                    ...
                    kind_name = site.specie.symbol + str(
                        ...
                        for m in unique_magmoms[site.specie.symbol]].index(True)
                    )
    ...
    def is_collin_mag(self):
        ...
        for kind in self.kinds:
            ...
            for kind_compare in self.kinds:
                ...
                if (np.linalg.norm(magmom_compare) -
                    np.dot(magmom, magmom_compare)/np.linalg.norm(magmom)) < _MAGMOM_THRESHOLD:
                    continue
                else:
                    return False
...
class Kind(oldKind):
    ...
    def compare_with(self, other_kind):
        ...
        try:
            diff = np.linalg.norm(
                np.array(self.magmom) - np.array(other_kind.magmom)
            ) > _MAGMOM_THRESHOLD
        except AttributeError:
            pass
        else:
            if diff:
                return (False, f'Magmoms are different ({self.magmom} vs. {other_kind.magmom})')
    ...
    def magmom(self, value):
        ...
        try:
            ...
            raise ValueError('Wrong format for magmom, must be a list of 3 float numbers.')
