...
class Wannier90BaseWorkChain(BaseRestartWorkChain):
    ...
    def define(cls, spec):
        ...
        spec.exit_code(400, 'ERROR_KMESH_TOL_FAILED', message='Not enough bvectors found even after reducing kmesh_tol.')
    ...
    def setup(self):
        ...
        self.ctx.kmesh_tol_trails = [self._WANNIER90_DEFAULT_KMESH_TOL, 1e-8, 1e-4]
    ...
    def handle_kmesh_tol(self, calc):
        ...
        if is_kmesh_tol:
            ...
            if len(self.ctx.kmesh_tol_trails) == 0:
                self.report('Not enough bvectors found after several trials of kmesh_tol')
                return ProcessHandlerReport(exit_code=self.exit_codes.ERROR_KMESH_TOL_FAILED)
            new_kmesh_tol = self.ctx.kmesh_tol_trails.pop(0)
            parameters[kmesh_tol_key] = new_kmesh_tol
            self.report('Not enough bvectors found, previous kmesh_tol: {}, trying a new kmesh_tol: {}'
