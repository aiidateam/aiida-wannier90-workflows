...
@options_core.ALL(help='Show all entries, regardless of their process state.', default=True)
...
def cmd_list(
    ...
    filters = builder.get_filters(
        all_entries=True, process_state=None, process_label=process_label, paused=False, exit_status=None, failed=False
    )
    query_set = builder.get_query_set(
        relationships=relationships, filters=filters, order_by={'ctime': 'asc'}, past_days=None, limit=None
    )
    state_projections = (
        'pk', 'state', 'process_state', 'process_status', 'exit_status', 'job_state', 'scheduler_state', 'exception'
    )
