...
@pytest.fixture
def generate_inputs_projwfc_base(
    ...
    def _generate_inputs_projwfc_base():
        ...
        inputs = {
            ...
            "parent_folder": generate_remote_data(
                fixture_localhost, fixture_sandbox.abspath, "quantumespresso.projwfc"
            ),
    ...
    def _generate_workchain_projwfc_base(exit_code=None, inputs=None, test_name=None):
        ...
        if exit_code is not None:
            ...
            node = generate_calc_job_node(
                entry_point_calc_job, fixture_localhost, test_name, inputs["projwfc"]
            )
...
def generate_workchain_pw2wannier90_base(
    ...
    def _generate_workchain_pw2wannier90_base(
        exit_code=None, inputs=None, test_name=None
    ):
...
def generate_inputs_wannier90_base(
    ...
    def _generate_inputs_wannier90_base():
        ...
        inputs = {
            ...
            "remote_input_folder": generate_remote_data(
                fixture_localhost, fixture_sandbox.abspath, "wannier90.wannier90"
            ),
            "metadata": {"options": get_default_options()},
        }
...
def generate_workchain_wannier90_base(
    ...
    def _generate_workchain_wannier90_base(exit_code=None, inputs=None, test_name=None):
        ...
        if exit_code is not None:
            ...
            node = generate_calc_job_node(
                entry_point_calc_job, fixture_localhost, test_name, inputs["wannier90"]
            )
            node.set_process_state(ProcessState.FINISHED)
            node.set_exit_status(exit_code.status)
...
def generate_inputs_wannier90(generate_inputs_pw, fixture_code):
    ...
    def _generate_inputs_wannier90():
        ...
        inputs = {
            ...
            "pw2wannier90": {"pw2wannier90": pw2wan},
            "wannier90": {"wannier90": w90},
        }
        return inputs
