...
def cmd_estimate_group(group, formatted, hdf_file):
    ...
    if os.path.exists(hdf_file):
        click.confirm(f'File {hdf_file} already exists, do you want to overwrite it?', abort=True)
...
@arguments_core.DATA(type=types.DataParamType(sub_classes=('aiida.data:structure',)), nargs=1)
@format_option
@decorators.with_dbenv()
