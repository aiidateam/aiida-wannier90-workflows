...
def get_pythtb(wan_calculation, xyz_workaround=False):
    ...
    if xyz_workaround:
        parsed = raw_wout_parser(open(os.path.join(out_dir, f'{seedname}.wout')).readlines())
        ...
        for wf in parsed['wannier_functions_output']:
            xyz.append(f"X {wf['coordinates'][0]} {wf['coordinates'][1]} {wf['coordinates'][2]}")
        for site in wan_calculation.inp.structure.sites:
            xyz.append(f'{site.kind_name} {site.position[0]} {site.position[1]} {site.position[2]}')
