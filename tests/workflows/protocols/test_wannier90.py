...
def test_spin_type(generate_builder_inputs):
    ...
    with pytest.raises(NotImplementedError):
        for spin_type in [SpinType.COLLINEAR, SpinType.NON_COLLINEAR]:
            builder = Wannier90WorkChain.get_builder_from_protocol(**generate_builder_inputs(), spin_type=spin_type)

    builder = Wannier90WorkChain.get_builder_from_protocol(**generate_builder_inputs(), spin_type=SpinType.NONE)
