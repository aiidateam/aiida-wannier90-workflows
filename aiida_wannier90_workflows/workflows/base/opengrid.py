...
class OpengridBaseWorkChain(BaseRestartWorkChain):
    ...
    def define(cls, spec):
        ...
        super().define(spec)
        spec.expose_inputs(OpengridCalculation, namespace='opengrid')
        ...
        spec.expose_outputs(OpengridCalculation)
    ...
    def setup(self):
        ...
        """
        super().setup()
        self.ctx.inputs = AttributeDict(self.exposed_inputs(OpengridCalculation, 'opengrid'))
    ...
    @process_handler(exit_codes=[OpengridCalculation.exit_codes.ERROR_OUTPUT_STDOUT_INCOMPLETE])  # pylint: disable=no-member
