...
def parse_w90_bands(band_dat, band_kpt, band_labelinfo, win):
    ...
    with open(band_dat) as handle:
        band_dat_lines = handle.readlines()
    with open(band_kpt) as handle:
        band_kpt_lines = handle.readlines()
    with open(band_labelinfo) as handle:
        band_labelinfo_lines = handle.readlines()

    bands, warnings = band_parser(band_dat_lines, band_kpt_lines, band_labelinfo_lines, struct)
    if len(warnings) != 0:
        print(warnings)
    return bands
