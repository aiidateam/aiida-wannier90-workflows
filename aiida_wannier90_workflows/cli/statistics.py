...
def cmd_optimize(group):
    ...
    for workchain in group.nodes:
        ...
        if workchain.exit_status != 0:
            echo.echo(f'{workchain.process_label}<{workchain.pk}> failed with exit_status {workchain.exit_status}')
            continue

        entry = [workchain.pk, workchain.inputs.structure.get_formula()]

        optimize_workchains = [
            _.node for _ in workchain.
            get_outgoing(link_type=LinkType.CALL_WORK, link_label_filter='wannier90_optimize_iteration%').all()
        ]
        num_optimization = len(optimize_workchains)
        entry.append(num_optimization)

        base_workchain = workchain.get_outgoing(link_type=LinkType.CALL_WORK, link_label_filter='wannier90').one().node
    ...
    headers = ['PK', 'structure', 'num_optimization', 'dis_proj_min', 'dis_proj_max', 'bands distance']
