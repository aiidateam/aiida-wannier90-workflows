...
from aiida.engine import ProcessBuilder, calcfunction, if_
...
class Wannier90BandsWorkChain(Wannier90OpenGridWorkChain):
    ...
    def results(self):
        ...
        if self.ctx.spin_collinear:
            if (
                "interpolated_bands" in self.outputs["wannier90_up"]
                and "interpolated_bands" in self.outputs["wannier90_down"]
            ):
                w90_bands = get_spinor_band_structure(
                    self.outputs["wannier90_up"]["interpolated_bands"],
                    self.outputs["wannier90_down"]["interpolated_bands"],
                    structure,
                )
        else:
...
def get_spinor_band_structure(  # pylint: disable=unused-argument,inconsistent-return-statements
    ...
    #   ├---k.set_cell <- structure from self.node.inputs.structure
